...
For someone who wants to run the app from source, change it, or reuse parts of it in another program. They describe the tree you are holding: version `1.11.2`, cut from commit `8d7f1d8` of the development repository (https://github.com/evilp0tat0/s201-aton-studio-dev). The per-change history, the long architecture reference and the commentary on the engineering rules live in that repository; this tree carries what the code needs and what a reader needs to understand it.
...
| `python dev/scripts/run-browser-smoke-gate.py` | Serves the tree on a free port, opens it headless in Chromium (Playwright — `pip install playwright && playwright install chromium` once), runs the whole self-test suite and asserts the suite size against `_COUNT_GROUND_TRUTH["smoke"]` (336 invariants). It refuses to report on a server that is not serving this tree's file. |
